...
export const savePlaces = async (db: SQLite.SQLiteDatabase, places: CachedPlace[]) => {
  const statement = await db.prepareAsync(
    'INSERT OR REPLACE INTO places_cache (place_id, name, lat, lng, photo_url, address, maps_url, cached_at) VALUES ($place_id, $name, $lat, $lng, $photo_url, $address, $maps_url, $cached_at)'
  );
  try {
